...
percentile_values AS (
    -- 第三步：定位每个分位点对应的消费金额
    SELECT
        MAX(CASE WHEN row_num = ROUND(0.99 * total_cnt) THEN total_spent END) AS p99,
        MAX(CASE WHEN row_num = ROUND(0.95 * total_cnt) THEN total_spent END) AS p95,
        MAX(CASE WHEN row_num = ROUND(0.90 * total_cnt) THEN total_spent END) AS p90,
        MAX(CASE WHEN row_num = ROUND(0.80 * total_cnt) THEN total_spent END) AS p80,
        MAX(CASE WHEN row_num = ROUND(0.70 * total_cnt) THEN total_spent END) AS p70,
        MAX(CASE WHEN row_num = ROUND(0.60 * total_cnt) THEN total_spent END) AS p60,
        MAX(CASE WHEN row_num = ROUND(0.50 * total_cnt) THEN total_spent END) AS p50,
        MAX(CASE WHEN row_num = ROUND(0.40 * total_cnt) THEN total_spent END) AS p40,
        MAX(CASE WHEN row_num = ROUND(0.30 * total_cnt) THEN total_spent END) AS p30,
        MAX(CASE WHEN row_num = ROUND(0.20 * total_cnt) THEN total_spent END) AS p20,
        MAX(CASE WHEN row_num = ROUND(0.10 * total_cnt) THEN total_spent END) AS p10,
        MAX(CASE WHEN row_num = ROUND(0.05 * total_cnt) THEN total_spent END) AS p5
    FROM ordered
)
-- 第四步：汇总量化结果，展示每个分位点的阈值及超出的用户数
SELECT '99%' AS percentile, p99 AS threshold,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p99) AS user_count
FROM percentile_values
UNION ALL
SELECT '95%', p95,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p95)
FROM percentile_values
UNION ALL
SELECT '90%', p90,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p90)
FROM percentile_values
UNION ALL
SELECT '80%', p80,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p80)
FROM percentile_values
UNION ALL
SELECT '70%', p70,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p70)
FROM percentile_values
UNION ALL
SELECT '60%', p60,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p60)
FROM percentile_values
UNION ALL
SELECT '50%', p50,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p50)
FROM percentile_values
UNION ALL
SELECT '40%', p40,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p40)
FROM percentile_values
UNION ALL
SELECT '30%', p30,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p30)
FROM percentile_values
UNION ALL
SELECT '20%', p20,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p20)
FROM percentile_values
UNION ALL
SELECT '10%', p10,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p10)
FROM percentile_values
UNION ALL
SELECT '5%', p5,
       (SELECT COUNT(*) FROM user_total_spent WHERE total_spent >= p5)
FROM percentile_values
